...
export const BackgroundMedia = ({ recording, siblingCover, isPlaying }) => {
  ...
  const videoAspectRatio = recording?.videoAspectRatio || '4:3';
  ...
  // Determine video fit styling based on aspect ratio chosen by Admin
  const getVideoFitClass = () => {
    if (videoAspectRatio === '4:3') {
      return 'w-full h-full object-contain max-h-screen aspect-[4/3]';
    }
    if (videoAspectRatio === '16:9') {
      return 'w-full h-full object-contain max-h-screen aspect-video';
    }
    if (videoAspectRatio === 'contain') {
      return 'w-full h-full object-contain max-h-screen';
    }
    // Default 'cover' (full-bleed 9:16)
    ...
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden select-none bg-black">
      <AnimatePresence mode="wait">
        {mediaType === 'video' && videoUrl ? (
          ...
          >
            {/* Pure original video with NO blur filter */}
            <video
              ref={videoRef}
              src={videoUrl}
              muted
              ...
              loop
              preload="auto"
              className={`relative z-10 transition-all duration-500 ${getVideoFitClass()}`}
              style={{ filter: 'none' }}
            />

            {/* Subtle soft gradient at top and bottom exclusively for navigation & scrubber contrast */}
          ...
          >
            <img
              ...
              className="relative z-10 w-full h-full object-cover md:object-contain brightness-95"
            />
            <div className="absolute inset-0 z-20 bg-gradient-to-b from-black/40 via-transparent to-black/70 pointer-events-none" />
          </motion.div>
        ) : (
